...
import com.lavy.redbook.user.relation.api.req.dto.CountFollowUnfollowMsg;
...
import com.lavy.redbook.user.relation.biz.enums.FollowUnfollowTypeEnum;
...
public class FollowUnfollowConsumer implements RocketMQListener<Message> {
    ...
    private void handleFollowTagMessage(String bodyJsonStr) {
        ...
        if (Boolean.TRUE.equals(isSuccess)) {
            ...
            // 发送MQ 该用户的粉丝数增加 统计关注数
            CountFollowUnfollowMsg msg = CountFollowUnfollowMsg.builder()
                    .userId(userId)
                    .targetUserId(followUserId)
                    .type(FollowUnfollowTypeEnum.FOLLOW.getType())
                    .build();
            doSendMqMsg(msg, userId, followUserId);
        }
    }
    ...
    private void handleUnfollowTagMessage(String bodyJsonStr) {
        ...
        if (Boolean.TRUE.equals(isSuccess)) {
            ...
            CountFollowUnfollowMsg msg = CountFollowUnfollowMsg.builder()
                    ...
                    .build();
            doSendMqMsg(msg, userId, unfollowUserId);
        }
    }
     ...
     */
    private void doSendMqMsg(CountFollowUnfollowMsg msg, Long userId, Long followUserId) {
        rocketMQTemplate.asyncSendOrderly(MQConstants.TOPIC_COUNT_FOLLOWING, msg, String.valueOf(userId),
                new SendCallback() {
                    @Override
                    public void onSuccess(SendResult sendResult) {
                        log.info("==> MQ {} 关注计数发送成功, {}", MQConstants.TOPIC_COUNT_FOLLOWING, msg);
                    }

                    @Override
                    public void onException(Throwable e) {
                        log.error("==> MQ {} 关注计数发送失败：{}", MQConstants.TOPIC_COUNT_FOLLOWING, msg, e);
                    }
                });
        rocketMQTemplate.asyncSendOrderly(MQConstants.TOPIC_COUNT_FANS, msg, String.valueOf(followUserId),
                new SendCallback() {
                    @Override
                    public void onSuccess(SendResult sendResult) {
                        log.info("==> MQ {} 粉丝计数发送成功, {}", MQConstants.TOPIC_COUNT_FANS, msg);
                    }

                    @Override
                    public void onException(Throwable e) {
                        log.error("==> MQ {} 粉丝计数发送失败：{}", MQConstants.TOPIC_COUNT_FANS, msg, e);
                    }
                });
    }
