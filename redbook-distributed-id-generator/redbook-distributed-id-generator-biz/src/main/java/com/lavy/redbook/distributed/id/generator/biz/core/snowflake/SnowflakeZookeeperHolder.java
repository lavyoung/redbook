package com.lavy.redbook.distributed.id.generator.biz.core.snowflake;
...
import com.lavy.redbook.distributed.id.generator.biz.core.snowflake.exception.CheckLastTimeException;
        ...
        try {
            ...
            if (stat == null) {
                ...
                if (workerid != null) {
                    ...
                    updateLocalWorkerID(workerID);
                    LOGGER.info(
                            "[Old NODE]find forever node have this endpoint ip-{} port-{} workid-{} childnode and "
                                    + "start SUCCESS",
                            ip, port, workerID);
                } else {
                    ...
                    updateLocalWorkerID(workerID);
                    LOGGER.info(
                            "[New NODE]can not find node on forever node that endpoint ip-{} port-{} workid-{},create"
                                    + " own node on forever node and start SUCCESS ",
                            ip, port, workerID);
                }
            }
        } catch (Exception e) {
